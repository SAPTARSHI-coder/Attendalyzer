...
Uses EXACT course code matching.
Format: Row1=Subject header (merged 3 cols), Row2=T-/A-/P-, Data from Row3
"""
...
def find_subject(subjects_list, target_code):
    """Return (total, present, pct) for a given code from OCR result list."""
    ...
    for s in subjects_list:
        ...
        if sc == tc_norm:
            ...
            ac  = min(ac, tc)   # cap attended at total
            pct = round(ac / tc * 100, 1) if tc > 0 else 0.0
            return tc, ac, pct
    return 0, 0, 0.0

def find_cache(name):
    """Fuzzy match name to a cache key."""
    # exact
...
for _, row in att_df.iterrows():
    ...
    rec = {"sl": int(sl), "name": name, "roll": roll}
...
# ── Create Excel ──────────────────────────────────────────────────────────────
OUT = "Attendance_Matrix_FINAL_v3.xlsx"
